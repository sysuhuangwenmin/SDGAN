...
class data_prefetcher():
    ...
    def preload(self):  #
        try:
            self.x, self.y,self.atts = next(self.iter)
        ...
        with torch.cuda.stream(self.stream):
            self.x, self.y,self.atts = self.x.cuda(self.gpu0, non_blocking=True), self.y.cuda(self.gpu0, non_blocking=True),self.atts.cuda(self.gpu0, non_blocking=True)

    def next(self):  #
        return self.x, self.y,self.atts
