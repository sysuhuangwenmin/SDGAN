...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style,bias):
        batch, in_channel, height, width = input.shape

        style = self.modulation(style).view(batch, 1, in_channel, 1, 1)
        style = style.view(batch, 1, in_channel, 1, 1)
        bias = bias.view(batch, 1, in_channel, 1, 1)
        weight = self.scale * self.weight * style#+ self.scale *bias
...
class StyledConv(nn.Module):
    ...
    def forward(self, input, noise=None):
        assert self.style is not None, "Please assign weight and bias before calling AdaIN!"
        assert self.bias is not None, "Please assign weight and bias before calling AdaIN!"
        out = self.conv(input, self.style,self.bias)
        #out = self.bias+out
        if self.noise:
...
class PositionEmbedding(nn.Module):
    def __init__(self, in_channels, out_channels, N_freqs=10, logscale=True):
        ...
        self.N_freqs = N_freqs  #9
        self.in_channels = in_channels  #2
        self.funcs = [torch.sin, torch.cos]
        self.out_channels = out_channels  #64
        if logscale:
            self.freq_bands = 2**torch.linspace(0, N_freqs-1, N_freqs) #1,2,4,8,16,32,64,128,256
        else:
            self.freq_bands = torch.linspace(1, 2**(N_freqs-1), N_freqs) #1-256之间线性插值
...
class FixedStyledConv(nn.Module):
    ...
    def __init__(
        ...
        self.upsample = upsample
        self.downsample = downsample
        #self.bias = nn.Parameter([1,torch.zeros(out_channel),1,1])
    ...
    def forward(self, input, style=None, noise=None):
        ...
        weight = self.modulation * self.scale * self.weight
        if self.demodulate:
...
class ToRGB(nn.Module):
    ...
    def forward(self, input, style, skip=None):
        ...
        # if skip is not None:
        #     if skip.size(2) < out.size(2):
        #         while skip.size(2) < out.size(2):
        #             skip = self.upsample(skip)
        #     else:
        #         while out.size(2) < skip.size(2):
